...
Pour réaliser cette carte, nous nous appuyons sur 5 packages : le package  `sf` [@sf] pour gérer les objets spatiaux,  le package `mapsf` [@mapsf] pour l'affichage des différentes couches composant la carte, le package `eurostat` [@eurostat] pour les données et le package `rnaturalEarth` [@rnaturalearth] pour quelques couches d'habillages supplémentaires. Le package `scales` [@scales] est uniquement utilisé pour réaliser le rééchelonnage d'une série statistique.
...
<b>Ainsi, l'utilisation de cette fonction doit ainsi obligatoirement être couplée avec la fonction</b>  `dev.off()`qui permet de fermer le périphérique graphique et de lancer l'enregistrement de son contenu dans un fichier.
...
La première étape pour cartographier les discontinuités spatiales est de récupérer les frontières entre chaque entités territoriales NUTS.
