...
```
Le fond de carte (ou couche géographique) crée est donc une fusion des découpages territoriaux suivants : 
...
land <- ne_download(
        ...
        returnclass = "sf")
```
Il est possible d'afficher de la couche d'habillage avec la fonction `mf_map` du package `mapsf`.
...
Nous pouvons ensuite découper toutes les couches d'habillage en fonction du rectangle préalablement crée.
...
Affichons de l'ensemble des couches recadrées et projetées pour voir le résultat :
...
A partir des différentes couches géographiques construites, recadrées et reprojetées, nous allons à présent réaliser un modèle de mise en page cartographique. 
...
On crée ensuite la fonction `template()` qui contiendra l'ensemble de la mise en page souhaitée. Ce modèle est construit à l'aide de fonctions du package `mapsf`.

<div class="alert alert-danger" role="alert">
Une fonction d'export de la carte est incluse dans `template()`. <b>Le résultat de cette fonction est donc une sortie graphique au format png, directement enregistrée sur votre machine.</b>
...
L'intégration d'un export automatique de la carte dans la fonction `template()` <b>permet de parfaitement contrôler les marges et le dimensionnement de la carte produite</b>, quelque soit la taille de sa fenêtre graphique.</div>
...
# Carte choroplèthe^[__La carte choroplèthe__ est le type le plus usuel de carte statistique. Il s’agit d’une représentation de quantités relatives à des espaces, ou aires géographiques, par le moyen d’une « échelle » de tons gradués.]
...
Nous pouvons ensuite facilement cartographier le PIB par habitant en utilisant la fonction de mise en page `template()` puis la fonction `mf_map()` pour représenter la variable souhaitée.
...
Pour cela, il suffit de réaliser une auto-intersection des NUTS (polygones de la couche 'nuts'). Afin d'éviter d'éventuelles erreurs topologiques, il est préférable de calculer une petite zone tampon autour des polygones avant réaliser l'intersection spatiale. Les géométries générées (lignes) doivent par la suite être définies en MULTILINESTRING. 
...
Nous pouvons maintenant effectuer une double jointure pour relier à chaque frontière NUTS les données de PIB par habitant des régions limitrophes.
...
# Double jointure pour récupérer les valeurs des deux NUTS limitrophes
...
$$
Discontinuité~relative = \frac{{PIB~par~habitant~de~la~région~A}}{{PIB~par~habitant~de~la~région~B}}
$$
...
Affichons les plus fortes discontinuités sélectionnées, avec la mise en page précédemment crée :
...
On effectue alors une boucle *for* qui construit 'nb' lignes, décalées de 'delta' à chaque fois.
...
Toutes les valeurs à représenter sont maintenant calculées et stockées dans l'objet 'disc'. Nous pouvons alors construire une fonction d'extrusion des lignes en fonction de ces valeurs.
...
Nous pouvons appliquer cette fonction à toutes les lignes en utilisant une boucle *for*. Voici un exemple d'utilisation de la fonction `extrude()`, combiné avec l'utilisation de `template()` et à l'**ajout d'une légende pour les discontinuités représentées**.
